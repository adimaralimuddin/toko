...
const store_ = create(set => ({
    products: [], updating: false,
    category: 'all', subCategory: 'all', maxPrice: 100000, minPrice: 0, onSale: false,
    ratings: -1, stock: -1, sold: -1, shipingFee: -1, originalPrice: -1,
    set: data => set(data)
}))
...
function useProduct() {
    const { products, set, updating, category, subCategory, maxPrice, minPrice, stock, sold, ratings, shipingFee, originalPrice, onSale } = store_()
    ...
    const getProductsByCategory = async (category) => {
        const { data } = await axios.get(url + `?type=category&category=${category}`)
        return data
    }
    ...
    const addProduct = async (store) => {
        ...
        const images = await Promise.all(
            store.imgFiles.map(async (file) => {
                ...
                const { data: { secure_url, public_id } } = await axios.post(curl, formData)
        ...
        set(p => ({ products: [...p?.products, res.data] }))
    ...
    return {
        set,
        products, updating, category, subCategory, minPrice, maxPrice, ratings, shipingFee, originalPrice, onSale,
        getAllProducts, getMainProduct, deleteById, addProduct, updateProduct,
