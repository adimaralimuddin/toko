...
function ProductAdder() {
    ...
    const { products, getAllProducts, addProduct, deleteById } = useProduct()
    ...
    const onAddHandler = async (e) => {
        ...
        const values = { ...Object.fromEntries(data.entries()), prices: store?.prices, extras: store?.extras }
    ...
    return (
        <div className='max-w-2xl mx-auto'>
            <form onSubmit={onAddHandler}
                className='flex flex-col p-2 bg-white m-2'>
                <h3>ADD PRODUCT</h3>
                <Input_
                    name='title'
                    value={store?.title}
                ...
                <div className='flex flex-wrap'>
                    ...
                    <Select_ name='freeShiping' label='Free Ship' values={[false, true]} />
                </div>
            ...
            <section>
                <div className='p-2'>
                    <div className='flex flex-wrap'>
                        {
                            store?.images?.map(
                                img => <img className='ring-1 mr-2 mb-2' width={100} src={img} />)
                        }
                    </div>
            ...
            <div className='flex flex-wrap'>
                {products?.map(product => <ProductAdminItem product={product} deleteById={deleteById} key={product?._id} />)}
            </div>
        </div>
    )
}
        ...
        <Link href={`/products/${product?._id}`}>
            <div className='flex flex-col bg-gray-200 p-2 m-2 w-[200px] cursor-pointer'>
                ...
                <div className='flex flex-wrap flex-1'>
                    {
                        product?.images?.map(
                            img => <img className='mr-1 ring-1' width={40} src={img.secure_url} />)
                    }
                </div>
                <span className='flex justify-between'>
                    <small>cat: <Dist>{product?.category}</Dist></small>
                    <small>subCat: <Dist>{product?.subCategory}</Dist></small>
                </span>
                <span className='flex items-center mb-1'>
                    <small>Prices:</small>
                    <div className='flex items-center'>{
                        product?.prices?.map(
                            price => <p className='px-1 ml-1 bg-gray-300 rounded-md'>{price}</p>
                        )
                    }</div>
                ...
                <footer>
                    <PrimaryButton onClick={() => deleteById(product?._id)}>delete</PrimaryButton>
...
function ExtraAdder({ store }) {
    ...
    const addExtraHandler = () => {
        ...
        store.set(p => ({ extras: [...p.extras, { text: p.text, price: p.extraPrice }] }))
    }
            ...
            <span className='flex flex-wrap'>
                {
                    store?.extras?.map(extra => (
                        <div
                            onClick={_ => store.set(p => ({ extras: p.extras.filter(p => p != extra) }))}
            ...
            <span>
                ...
                <div className='flex flex-wrap'>
                    {store?.prices?.map(price =>
                        <p onClick={_ => store.set(p => ({ prices: p.prices.filter(p => p != price) }))} className=' p-1 px-2 m-1 text-sm rounded-md bg-gray-200'>
                            {price}
                        </p>)}
                </div>
            </span>
            <button onClick={_ => store.set(p => ({ prices: [...p.prices, p.price] }))} className='px-2 py-1 rounded-md bg-purple-500 mx-1 text-white'>Add</button>
        </div>
    )
}
