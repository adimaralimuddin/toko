...
function AdminProductDetails({ product }) {
  ...
  return (
    <div>
      ...
      <footer>
        <hr />
        <button onClick={(_) => store.set((p) => ({ ...p?.original }))}>
          Reset
        </button>
    ...
    <div>
      <div className="flex flex-wrap">
        {prices?.map((price) => (
          <div
            onClick={(_) =>
              set((p) => ({ prices: p.prices?.filter((p_) => p_ != price) }))
            }
            className="p-1 m-1 bg-gray-200"
            key={price}
          >
            {price}
          </div>
      ...
      <div>
        <button
          onClick={(_) =>
            set((p) => ({ prices: [...p?.prices, p?.curPrice], curPrice: "" }))
          }
        >
          Add Price
        </button>
        <input
          value={curPrice}
          onChange={(e) => set({ curPrice: e.target?.value })}
          type="number"
        />
      </div>
    </div>
...
function Images({ images }) {
  return (
    <div>
      {images?.map((image) => (
        <img src={image?.secure_url} width={200} key={image?.secure_url} />
      ))}
    </div>
  );
}
