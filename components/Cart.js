...
import { Badge, Button, Checkbox, Divider, Drawer, IconButton, List, ListItem, ListItemButton, ListItemIcon, ListItemText, Menu, Stack, Typography } from '@mui/material';
...
function Cart() {
    ...
    const { cart, selectItem, toCheckout,
        deletePayItem, getSubTotal, incPayItemQnty,
        decPayItemQnty, getTransactions } = useCart()
                ...
                >
                    ...
                    <Stack direction='row' justifyContent="space-between" >
                        <span className='flex items-center'>
                            <Checkbox />
                            <Typography variant='body2'>Select All ({quantity} item(s))</Typography>
                        </span>
                        <span>
                    ...
                    <footer>
                        <span className='text-gray-600 flex flex-wrap justify-between'>
                            <Typography variant='body1' mx color=''>
                                ...
                                <span className='text-orange-500 '>${getSubTotal()
                                }</span></ Typography>
                            <Typography variant='body1' mx>Shiping fees: ${total}</ Typography>
                        </span>
...
function ProductItem({ product, selectItem, deletePayItem, incPayItemQnty, decPayItemQnty, deselect }) {
    ...
    return (
        <div className='flex border-b py-2 sm:flex-column content-right flex-wrap items-center justify-between hover:bg-gray-100 p-1' >
            <div className='flex items-center flex-1 min-w-[60%]'            >
                <Checkbox size='small' checked={product?.checked} onChange={onSelectHandler} />
                <img
                    className=''
                    src={product?.image} width='50' alt="" />
                <Typography ml variant='body2' >{product?.name}</Typography>
            </div>

            <div className='flex flex-wrap'>

                <Typography variant='body2' color='primary' mr>${product?.curPrice}</Typography>
                <QuantityEditor
                    increase={_ => incPayItemQnty(product?._id)}
                    decrease={_ => decPayItemQnty(product?._id)}
                    quantity={product?.quantity} simple='true' />
                <IconButton sx={{ marginLeft: 1 }}>
                    <DeleteOutlineOutlinedIcon size='small' onClick={_ => deletePayItem(product?._id)} />
                </IconButton>
            </div>
        </div>
    )
